...
router.get('/leaderboard', auth, (req, res) => {
  ...
  const rows = db.prepare(`
    SELECT u.id, u.username, u.display_name, u.avatar_color, u.avatar_url, u.is_admin,
      ...
      COALESCE(mp.pts,0)+COALESCE(gp.gp,0)+COALESCE(pp.pts,0) AS total_points,
    ...
    LEFT JOIN (SELECT user_id, SUM(points_earned) gp FROM group_points GROUP BY user_id) gp
           ON gp.user_id=u.id
    LEFT JOIN (SELECT user_id, SUM(amount) pts FROM point_transactions WHERE type!='admin_adjustment' GROUP BY user_id) pp
           ON pp.user_id=u.id
    WHERE u.id IN (${ph})
